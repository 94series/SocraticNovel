...
>
> 教材和大部头著作翻不下去，公开课太单向，问 AI 它直接把答案从头到尾替你算完——怎么学都是一个人在硬撑。SocraticNovel 是一个开源框架，让你创建**沉浸式 AI 教学系统**：AI 不直接给答案，用苏格拉底式提问引导你一步步自己推出来；教学不发生在空白聊天框里，而是在一个有角色、有场景、有日常的轻小说世界中。
>
...
🔥 **T0 · 毫无保留推荐**
- **Claude Sonnet/Opus 4.x** — 当之无愧的文学素养天花板。文笔极其克制，能写出"用光线微变替代好感度"那种幽微的文学感。读写文件系统原生支持，就像是为这套系统量身定做的模型
- **GPT-5.x** — 逻辑推导极其精确，公式解题几乎零失误。叙事上不如 Claude 细腻，但"教学"这条线拉得很稳

🌟 **T1 · 国产之光**
- **DeepSeek-V3.x** — 文字张力的巅峰之一，不仅极其聪明，而且非常听话。用它跑这套系统性价比极高
- **Kimi (200K+)** — 变态级别的长文本吞吐量。把 15 个文件全丢给它的知识库，聊上一个月都不会失忆
...
我所做的是对他们的 idea 进行逆向工程化、架构抽象和设计迭代，将其提炼为一个可复用的开源框架。
